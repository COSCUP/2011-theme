...
function m(b,d){var f=a("<ul />"),g=[];if(d)for(var e=0,c;e<2;){c=d.results.shift();if(!c)break;!/plurk\.com/.test(c.source)&&!/^RT/.test(c.text)&&a.inArray(c.from_user,g)===-1&&(g.push(c.from_user),f.append(a("<li />").append(a('<span class="text" />').html(c.text)).append('<span class="meta"><a href="https://twitter.com/#!/'+c.from_user+"/status/"+c.id_str+'">@'+c.from_user+"</a></span>")),e++)}if(b)for(e=0;e<2;){c=b.plurks.shift();if(!c)break;b.users[c.user_id]&&!/plurk\.com\/\p\//.test(c.content)&&
